...
pub fn build(
    ...
    const upstream = try b.dependencyLazy("zstd_src", .{});

    const linkage = b.option(std.builtin.LinkMode, "zstd.linkage", "Link mode") orelse .static;
    const strip = b.option(bool, "zstd.strip", "Omit debug information");
    const pic = b.option(bool, "zstd.pie", "Produce Position Independent Code");

    const compression = b.option(bool, "zstd.compression", "build compression module") orelse true;
    const decompression = b.option(bool, "zstd.decompression", "build decompression module") orelse true;
    const dictbuilder = b.option(bool, "zstd.dictbuilder", "build dictbuilder module") orelse compression;
    const deprecated = b.option(bool, "zstd.deprecated", "build deprecated module") orelse false;

    const minify = b.option(bool, "zstd.minify", "Configures a bunch of other options to space-optimized defaults") orelse false;
    const legacy_support = b.option(usize, "zstd.legacy-support", "makes it possible to decompress legacy zstd formats") orelse @as(usize, if (minify) 0 else 5);
    ...
    const multi_thread = b.option(bool, "zstd.multi-thread", "Enable multi-threading") orelse (target.result.os.tag != .wasi);
    const disable_assembly = b.option(bool, "zstd.disable-assembly", "Assembly support") orelse false;
    const huf_force_decompress_x1 = b.option(bool, "zstd.huf-force-decompress-x1", "") orelse minify;
    const huf_force_decompress_x2 = b.option(bool, "zstd.huf-force-decompress-x2", "") orelse false;
    const force_decompress_sequences_short = b.option(bool, "zstd.force-decompress-sequences-short", "") orelse minify;
    const force_decompress_sequences_long = b.option(bool, "zstd.force-decompress-sequences-long", "") orelse false;
    const no_inline = b.option(bool, "zstd.no-inline", "Disable Inlining") orelse minify;
    const strip_error_strings = b.option(bool, "zstd.strip-error-strings", "removes the error messages that are otherwise returned by `ZSTD_getErrorName` (implied by `-Dminify`)") orelse minify;
    const exclude_compressors_dfast_and_up = b.option(bool, "zstd.exclude-compressors-dfast-and-up", "") orelse false;
    const exclude_compressors_greedy_and_up = b.option(bool, "zstd.exclude-compressors-greedy-and-up", "") orelse false;
